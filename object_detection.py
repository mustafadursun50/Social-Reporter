...
def draw_person_bbox(img, person_box, labels, confidence, colors=None, write_conf=False):
    ...
    for i, label in enumerate(labels):
        ...
        cv2.rectangle(img, (person_box[i][0],person_box[i][1]), (person_box[i][2],person_box[i][3]), color, 2)
        cv2.putText(img, label, (person_box[i][0],person_box[i][1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

    return img

def draw_env_bbox(img, person_box, labels, confidence, colors=None, write_conf=False):
    """A method to apply a box to the image
    Args:
        img: An image in the form of a numPy array
        bbox: An array of bounding boxes
...
def detect_common_objects(image, confidence=0.5, nms_thresh=0.3, enable_gpu=False):
    ...
    for i in indices:
        ...
        if(str(classes[class_ids[i]]) == 'person'):
            person_box.append([int(x), int(y), int(x+w), int(y+h)])
            person_label.append(str(classes[class_ids[i]]))
            conf.append(confidences[i])
        else:
            env_box.append([int(x), int(y), int(x+w), int(y+h)])
            env_label.append(str(classes[class_ids[i]]))
            conf.append(confidences[i])

        
    return person_box, env_box, person_label, env_label, conf
